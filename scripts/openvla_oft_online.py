...
class OpenVLAOFTCapture(Node):
    def __init__(self):
        ...
        self.image_topic = self.declare_parameter('image_topic', '/camera/camera/color/image_raw').get_parameter_value().string_value
        ...
        self.finger_names = self.declare_parameter('finger_joints',
            ['finger_1_joint','finger_2_joint','finger_3_joint']).get_parameter_value().string_array_value
        self.open_pos   = list(self.declare_parameter('finger_open',  [0.0,0.0,0.0]).get_parameter_value().double_array_value)
        self.closed_pos = list(self.declare_parameter('finger_closed',[1.0,1.0,1.0]).get_parameter_value().double_array_value)
